...
| `--publicationMode` | `preserve-layers`, `collapse-draft`, or `save-only` | For `copy stories`, preserve Storyblok publication state after full rewritten content is saved. Default: `preserve-layers`. |
| `--publicationLanguages` | `default`, `all`, or comma-separated codes | Language scope to publish when `publicationMode` publishes stories. Default: `all`. |
...
Story applies create or match unpublished target shells first, write the
source-to-target manifest, then save the full copied content with manifest-backed
asset and story reference rewrites. Shells are always created as save-only drafts
so placeholder shells are never published.

`copy stories` then applies `--publicationMode`. `preserve-layers` is the
default: clean published source stories are published in the target; dirty
published source stories copy the source published version, publish that target
layer, then restore the source draft/current layer as save-only. `collapse-draft`
publishes published source stories from their current source JSON. `save-only`
never publishes copied stories and rejects `--publicationLanguages`.
...
detected from asset URLs without trying to copy from those spaces. Large
referenced-asset plans log story scan progress, a reference planning summary,
and target conflict-check progress so long dry-runs do not appear idle while
the graph and target-path checks are running. With
