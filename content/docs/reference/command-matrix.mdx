...
Reference: [copy](/docs/reference/copy). As of `sb-mig@6.5.0-beta.17`. `copy stories`, `copy relink` and `copy assets` also take `--progress auto|line|plain|off` and `--verbose`. `copy space --only` accepts `languages`, `settings`, `groups`, `components`, `presets`, `datasources`.

| Command form | Required selector | Optional flags | Writes Storyblok |
| --- | --- | --- | --- |
| `sb-mig copy space --from <space> --to <space>` | `--from`, `--to` (different spaces) | `--only`, `--allow-missing-plugins`, `--dry-run`, `--yes`, `--outputPath` | Yes unless `--dry-run`; never deletes |
| `sb-mig copy stories --source <story> --destination <folder>` | `--source` | `--from`, `--to`, `--destination`, `--mode`, `--with-assets`, `--publicationMode`, `--publicationLanguages`, `--fresh`, `--manifestRoot`, `--dry-run`, `--yes`, `--outputPath`, `--progress`, `--verbose` | Yes unless `--dry-run` |
| `sb-mig copy stories --source <folder> --destination <folder>` | `--source` | same as above | Yes unless `--dry-run` |
| `sb-mig copy stories --source <folder>/* --destination <folder>` | `--source` | same as above (`--mode` defaults to `children`) | Yes unless `--dry-run` |
| `sb-mig copy stories --source <a> --source <b> ...` or `--source a,b` | `--source` (repeatable) | same as above | Yes unless `--dry-run` |
| `sb-mig copy stories --source / --destination /` | `--source /` | same as above plus `--exclude` | Yes unless `--dry-run`; every root story and folder of the source |
| `sb-mig copy relink --source <story-or-folder> --destination <folder>` | `--source` | `--from`, `--to`, `--destination`, `--mode`, `--publicationMode` (`preserve-layers`, `save-only`), `--publicationLanguages`, `--manifestRoot`, `--dry-run`, `--yes`, `--outputPath`, `--progress`, `--verbose` | Yes unless `--dry-run`; reference values only, publication state kept |
| `sb-mig copy relink --source / --destination /` | `--source /` | same as above plus `--exclude` | Yes unless `--dry-run`; reference values only, publication state kept, whole space |
| `sb-mig copy assets --all` | `--all` | `--from`, `--to`, `--manifestRoot`, `--dry-run`, `--outputPath`, `--progress`, `--verbose` | Yes unless `--dry-run` |
| `sb-mig copy assets --asset <id-url-or-file>` | `--asset` (repeatable) | `--from`, `--to`, `--manifestRoot`, `--dry-run`, `--outputPath`, `--progress`, `--verbose` | Yes unless `--dry-run` |
| `sb-mig copy assets --assetFolder <id-or-path>` | `--assetFolder` (repeatable) | `--from`, `--to`, `--manifestRoot`, `--dry-run`, `--outputPath`, `--progress`, `--verbose` | Yes unless `--dry-run` |
| `sb-mig copy assets --referenced-by-stories --source <story-or-folder>` | `--referenced-by-stories`, `--source` | `--from`, `--to`, `--mode`, `--manifestRoot`, `--dry-run`, `--outputPath`, `--progress`, `--verbose` | Yes unless `--dry-run` |
| `sb-mig copy manifests` | none | `--manifestRoot` | No |
| `sb-mig copy manifests --pair <source>:<target>` | `--pair` | `--type`, `--slug`, `--manifestRoot`, `--outputPath` | No |
| `sb-mig copy manifests --prune <source>:<target>` | `--prune` | `--dry-run`, `--yes`, `--manifestRoot`, `--outputPath` | No; deletes the ledger directory on disk |

Copy constraints:

| Combination | Status |
| --- | --- |
| `copy space --from A --to A` | Rejected. Two different spaces are required. |
| `copy space` into a space missing field-type plugins the source uses | Refuses to write; `--allow-missing-plugins` writes and reports the rejected components. |
| `copy stories --publicationMode save-only --publicationLanguages ...` | Rejected. Save-only publishes nothing. |
| `copy stories --destination <missing folder>` | Rejected. The destination must exist, or be omitted, `/` or `root`. |
| `copy stories` or `copy relink` or `copy space` without a terminal and without `--yes` | Refuses to write after printing the PLAN block. |
| `copy relink --fresh` | Not a flag. The ledger is relink's input. |
| `copy relink --publicationMode collapse-draft` | Rejected: it would publish unfinished drafts. Use `preserve-layers` (the default) or `save-only`. |
| `copy stories`, `copy relink` or `copy assets` with `--progress` other than `auto`, `line`, `plain`, `off` | Rejected before anything is read. |
| `copy stories --source / --mode children` | Rejected. `/` already means everything under the space root. |
| `--exclude` without `--source /`, a name that is not a root, a root both named and excluded, or every root excluded | Rejected before any read, naming the roots. |
| `copy assets` with two selector families (`--all --asset ...`) | Rejected. One selector family per run. |
| `copy manifests --pair` with a non-numeric space id, or half a pair | Rejected. Never falls back to the configured `spaceId`. |
| `copy manifests --prune` with `--pair`, `--from`, `--to`, `--type` or `--slug` | Rejected. `--prune` names its own pair. |
| `copy manifests --prune` on a path with a symbolic link, or `--outputPath` inside the pruned directory | Refused. |
